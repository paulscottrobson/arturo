...
static uint8_t parityFlagValue[256];												// A look up table for the P Flag for ALU etc
...
static uint8_t ConstructStatusByte(void) {
	uint8_t f = 0;												
...
static void ExplodeStatusByte(uint8_t f) {
...
static inline uint16_t _Fetch8Signed(void) { 										// Support function.
	uint16_t w1 = FETCH8();
...
static inline void _Push(uint16_t d) {
	SP -= 2;
	WRITE16(SP,d);
}

static inline uint16_t _Pop(void) {
	uint16_t d = READ16(SP);
	SP += 2;
	return d;
}
...
static uint8_t _add(uint8_t n,uint8_t c) {
	temp16 = A + n + c;
...
static uint8_t _sub(uint8_t n,uint8_t c) {
	temp16 = A - n - c;
	c_Flag = (temp16 & 0x100) != 0;
	n_Flag = 1;
	SETNZ(temp16);
...
static uint16_t add16(uint16_t r1,uint16_t r2) {
	long r = r1 + r2;
...
static void _setNZ16(uint16_t w) {
	if (w & 0x8000) {
		SETNZ(0x80);
	} else {
		SETNZ(w == 0 ? 0 : 1);
...
static uint16_t adc16(uint16_t r1,uint16_t r2) {
...
static uint16_t sbc16(uint16_t r1,uint16_t r2) {
	temp32 = r1 - r2 - (c_Flag ? 1 : 0);
	_setNZ16(temp32 & 0xFFFF);
...
static uint8_t _RotateMake(uint8_t v,uint8_t c) {
	SETNZ(v);
	SETHALFCARRY(0);
	SETPARITY(v);
	SETNFLAG(0);
...
static void bitOp(uint8_t test) {
	SETNZ(test);
	SETHALFCARRY(1);
	SETNFLAG(0);
	SETPARITY(test);
