//
//    In the tree, so the file scan picks it up, but I want it here :) So this requires COMPILEME to be defined otherwise it looks empty.
//
#ifdef COMPILEME
/*
...
int main(int argc, char *argv[]) {
  ...
  printf("const FONTGlyph %sGlyphs[] = {\n", fontName);
  for (i = first, j = 0; i <= last; i++, j++) {
  ...
  printf("const FONTInfo %s = {\n", fontName);
  printf("  (uint8_t  *)%sBitmaps,\n", fontName);
  printf("  (FONTGlyph *)%sGlyphs,\n", fontName);
